...
class graphDB:
    def __init__(self, uri, user, password):
        ...
        self.neo = neointerface.NeoInterface(host=self.uri , credentials=(self.user, self.password))
        self.graph = Graph(f"http://" + self.user + ":" + self.password + "@localhost:7474/db/data/")
    ...
    def add(self, file):
        ...
        # create index
        try:
            self.graph.run("CREATE INDEX ON :object(video_id);")
            # print("index ok")
        ...
        old_count = self.graph.run(old_q)
        ...
        new_count = self.graph.run(new_q)
